(function ($, window, document, undefined) {
  ...
  const mobileMenu = () => {
    if (window.outerWidth <= 991 && $(".unit").length) {
      if (!$(".menu-item-has-children i").length) {
        $(".site-content__mobile-menu .menu-item-has-children").append(
          '<i class="fa fa-angle-down js-btn-mobile"></i>'
        );
      }
  ...
  const addWrapperToElements = () => {
    ...
    if (
      $(
        ".site-content__sidebar select, .wp-block-archives select, .wp-block-categories select"
      ).length
    ) {
      $(
        ".site-content__sidebar select, .wp-block-archives select, .wp-block-categories select"
  ...
  const ajaxLoadPost = (postID) => {
    ...
    if (savedPostLoadedAjax.hasOwnProperty(postID) === false) {
      ...
      $(".site-content__page-wrap .mCSB_container").html(
        savedPostLoadedAjax[postID]
      );
